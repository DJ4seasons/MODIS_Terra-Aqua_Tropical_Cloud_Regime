...
map0t=np.zeros([nk,nlat,nlon],dtype=float)
map0a=np.zeros([nk,nlat,nlon],dtype=float)
...
for yy in range(iyr,eyr+1,1):
    ...
    for i in range(1,nk+1,1):
        if i<nk:
            ...
            idx1 = np.logical_and(crnum_t>100,crnum_t<=100+sub_nk)
            idx2 = np.logical_and(crnum_a>100,crnum_a<=100+sub_nk)
...
grfoobst=map0t.reshape([nk,nlat*nlon]).mean(axis=1)
grfoobsa=map0a.reshape([nk,nlat*nlon]).mean(axis=1)
print(grfoobst)
print(grfoobsa)

map0= (map0t+map0a)/2.0
grfoobsall= map0.reshape([nk,nlat*nlon]).mean(axis=1)
...
plt.suptitle("MODIS C6.1 T+A Tropical CR RFO Map",fontsize=18,y=1.0)
lf=0.05;rf=0.95
bf=0.03;tf=0.95
...
for ii in range(nk):
    ...
    # add a title.
    subtit='CR{} [CF={:.1f}%]: RFO={:.1f}% [T{:.1f}%, A{:.1f}%]'.format(ii+1,obscf[ii],grfoobsall[ii],grfoobst[ii],grfoobsa[ii])
    print(subtit)
